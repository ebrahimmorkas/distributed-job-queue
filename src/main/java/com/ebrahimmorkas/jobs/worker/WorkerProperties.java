 ...
 * @param retryBaseDelay  delay before the first retry (doubles each attempt, with jitter)
 * @param retryMaxDelay   upper bound for the retry delay
...
public record WorkerProperties(boolean enabled, int concurrency, Duration pollInterval, Duration lease,
                               Duration retryBaseDelay, Duration retryMaxDelay, Duration shutdownTimeout) {
}
